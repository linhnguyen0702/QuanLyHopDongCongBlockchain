...
const EditContractor = () => {
  ...
  const updateContractorMutation = useMutation(
    ({ id, contractorData }) => contractorAPI.updateContractor(id, contractorData),
    {
      onSuccess: (response) => {
        queryClient.invalidateQueries('contractors');
        queryClient.invalidateQueries(['contractor', id]);
        toast.success('Cập nhật nhà thầu thành công!');
        navigate('/contractors');
      },
      onError: (error) => {
        console.error('Update contractor error:', error);
        toast.error(error.response?.data?.message || 'Cập nhật nhà thầu thất bại!');
      }
    }
  );

  // Update form data when contractor data is loaded
  useEffect(() => {
    if (contractorResponse) {
      setFormData({
        contractorCode: contractorResponse.contractorCode || '',
        contractorName: contractorResponse.contractorName || '',
        contactPerson: contractorResponse.contactPerson || '',
        email: contractorResponse.email || '',
        phone: contractorResponse.phone || '',
        address: contractorResponse.address || '',
        taxCode: contractorResponse.taxCode || '',
        businessLicense: contractorResponse.businessLicense || '',
        contractorType: contractorResponse.contractorType || 'construction',
        status: contractorResponse.status || 'active',
        rating: contractorResponse.rating || 3,
  ...
  if (!isManager) {
    return (
      <Box>
        ...
        <Alert severity="error">
          Bạn không có quyền chỉnh sửa nhà thầu. Chỉ quản lý và quản trị viên mới có thể thực hiện chức năng này.
        </Alert>
      </Box>
    );
  }
          ...
          <form onSubmit={handleSubmit}>
            ...
            <Box sx={{ display: 'flex', gap: 2, mt: 4, justifyContent: 'flex-end' }}>
              <Button
                ...
                disabled={updateContractorMutation.isLoading}
              >
                Hủy
              </Button>
              <Button
                type="submit"
                variant="contained"
                startIcon={updateContractorMutation.isLoading ? <CircularProgress size={20} /> : <SaveIcon />}
                disabled={updateContractorMutation.isLoading}
              >
                {updateContractorMutation.isLoading ? 'Đang cập nhật...' : 'Cập nhật nhà thầu'}
              </Button>
            </Box>
          </form>
        </CardContent>
